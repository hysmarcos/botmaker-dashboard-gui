...
if "switch_tab" not in st.session_state:
    st.session_state["switch_tab"] = False
...
with tab_upload:
    ...
    if uploaded_users and uploaded_operators_sessions:
        ...
        if df_users_raw is not None and df_sessions_raw is not None and validate_dataframes(df_sessions_raw, df_users_raw):
            st.session_state["df_procesado"] = clean_and_prepare_data(df_sessions_raw.copy(), df_users_raw.copy())
    ...
    if st.session_state.get("data_ready"):
        ...
        if df_filtrado.empty:
            ...
            with chart_cols[1]:
                ...
                fig_aht.update_traces(textposition='outside')
        ...
        st.info("👋 Primero carga los archivos desde la pestaña 'Carga de archivos'.")
